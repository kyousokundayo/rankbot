...
# 投票・フェーズ境界の server mute は、上の汎用一括更新と同じ列で直列化するが、
# APIクライアントが429を正しく待つことを前提に、間隔だけを短くする。13人全員の
# ミュートを1.1秒刻みで送ると操作だけで14秒以上を要し、投票開始が実用にならない。
# 共有ロックは維持するため、他のメンバー/権限更新と並列に突っ込むことはない。
VOICE_MUTE_API_INTERVAL = 0.5
