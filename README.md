...
- 現在のバージョン: **v0.40**（`config.py` の `BOT_VERSION`、`SPEC.md` の対応Bot、`CHANGELOG.md` とここを更新時に揃える）
...
既存DBは起動時にv0.40の必須テーブル・列型・NULL可否・DEFAULT・主キー・
一意制約・外部キー・CHECK制約と、
ラダー・精算・未終了募集の整合性を検証し、
未移行または不整合な状態なら変更せず起動を中止します。旧版DBからの直接アップグレードはサポートしないため、
v0.40へ移行済みのバックアップまたは新規DBを使用してください。
移行済みDBに残る未使用の旧列・空テーブル・完了markerは現行コードから参照せず、
旧テーブル内の整合性も起動条件にせず、起動時には自動削除しません。物理削除は停止中の個別DBメンテナンスとして扱います。
