...
- 現在のバージョン: **v0.37**（`config.py` の `BOT_VERSION` が正。更新時は `CHANGELOG.md` とここも揃える）
...
- 初心者・中級者・上級者・村長ロール説明は当面Administrator権限保持者だけに表示。9人の2卓は **ねいと** ロール限定（終了ログも公開ログへ退避せず削除）、13人ターン制はRunner・カテゴリ・VC・受付・募集を作らない未公開状態。新規変種は公開`#募集`へのカード作成も停止（既存の制限卓募集は従来どおり利用可）
...
- `#募集` で7日先までの13人/9人募集を作り、参加・補欠・GMを再起動後も保持して既存ロビーへ一括移行
- 卓の予約（13人ターンは150分、ほかは90分）、補欠の自動繰り上げ、15分前DM、開催時のランク・GM条件再確認に対応
...
- 13人村 `l13` / 9人村 `l9` の2ラダー。変種別固定プール + 卓帯補正 + 活躍ボーナス + エメラルドを含む相対ランク9段階 + 共通シーズン境界
- 終了後、霊媒師・初日処刑者・初夜襲撃死者が匿名で参加者へレート+1を贈る推薦
- 終了時に**全役職の行動・投票・死亡を発生順にまとめた進行ログ**を `#昼` へ掲示（拒否された操作も残るため二重実行の有無をその場で確認できる）
- 通常卓で終了した `#昼` / `#霊界` は**ログカテゴリへ試合番号付きで退避**（`04-昼`。書き込み不可・最大50試合ぶんで、上限に達すると古い順に40試合ぶんまで整理。退避に失敗した場合のみ削除）。ねいと限定の9人卓は公開ログに漏らさず削除
...
`WEREWOLF_LOCAL_ROOMS_JSON`を参考に`.env`へ追加してください。標準構成は
初心者・中級者・上級者、13人クロストークの総合、ねいとロール限定の9人クロストーク/
9人ターン制の計6卓です。13人ターン制は実装済みですが、起動時にカテゴリ・VC・受付を
作らない未公開状態です。9人の2卓は直接ロビーで試験し、公開`#募集`には出しません。
...
| 募集の占有時間・通知・上限・ランク選択肢 | `VARIANT_DEFINITIONS[*].recruitment_occupancy_minutes` / `RECRUITMENT_*` / `PLAYER_BLOCK_LIMIT` |
...
| ねいとロール限定の試験卓 | `RoomDefinition.strict_access_role_names` |
| 段階導入中だけ募集を止める新規卓 | `RECRUITMENT_DISABLED_ROOM_IDS` |
