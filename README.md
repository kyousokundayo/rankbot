...
- 現在のバージョン: **v0.38**（`config.py` の `BOT_VERSION` が正。更新時は `CHANGELOG.md` とここも揃える）
...
- 初心者・中級者・上級者・GMロール説明は当面Administrator権限保持者だけに表示。9人の2卓は一般公開し、`#募集`にも出す。13人ターン制はRunner・カテゴリ・VC・受付・募集を作らない未公開状態
...
- `#募集` はGMまたは仮GMが7日先まで作成でき、参加・補欠・GMを再起動後も保持して既存ロビーへ一括移行
...
- 公開卓で終了した `#昼` / `#霊界` は**ログカテゴリへ試合番号付きで退避**（`04-昼`。書き込み不可・最大50試合ぶんで、上限に達すると古い順に40試合ぶんまで整理。退避に失敗した場合のみ削除）
...
初心者・中級者・上級者、13人クロストークの総合、9人クロストーク/
9人ターン制の計6卓です。13人ターン制は実装済みですが、起動時にカテゴリ・VC・受付を
作らない未公開状態です。9人の2卓は直接ロビーと`#募集`で利用できます。
...
| 当面管理者だけに見せる卓・GM説明 | `ADMIN_ONLY_ROOM_IDS` / `GM_INFO_ADMIN_ONLY` |
| 厳格ロール限定の固定卓 | `RoomDefinition.strict_access_role_names` |
| 未公開卓の募集停止 | `RECRUITMENT_DISABLED_ROOM_IDS` |
