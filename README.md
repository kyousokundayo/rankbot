...
- 現在のバージョン: **v0.35**（`config.py` の `BOT_VERSION` が正。更新時は `CHANGELOG.md` とここも揃える）
...
- **議論中から投票できる**（投票フェーズまでは何度でも入れ替え可）。生存者全員が入れ終わると議論を切り上げ、その票でそのまま開示
- 夜は時間切れでは明けず、**`#昼` のパネルで生存者全員が「朝を迎える」を押すと朝**になる（宣言人数は夜の間は誰にも見せず、目安時間が切れたときに全員へ同時に公開。GMのみGMメニューの「朝」で強制可）
...
- 固定プール制レート + 卓帯補正 + 活躍ボーナス + エメラルドを含む相対ランク9段階 + シーズン制
- 終了後、霊媒師・初日処刑者・初夜襲撃死者が匿名で参加者へレート+1を贈る推薦
- 終了時に**全役職の行動・投票・死亡を発生順にまとめた進行ログ**を `#昼` へ掲示（拒否された操作も残るため二重実行の有無をその場で確認できる）
- 終了した `#昼` / `#霊界` は削除せず**ログカテゴリへ試合番号付きで退避**（`04-昼`。書き込み不可・最大50試合ぶんで、上限に達すると古い順に40試合ぶんまで整理。退避に失敗した場合のみ従来どおり削除）
